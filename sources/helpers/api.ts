...
const APIConfigs: APIConfigsType = {
  ...
  List: ['GET', '/note/list'],
  Save: ['POST', '/note/save'],
  Delete: ['POST', '/note/delete'],
  Update: ['POST', '/note/update'],
};
...
type ConfigsType = ['GET' | 'POST', string];
...
function GenerateAPIsFromAPIParamsType() {
  ...
  for (const key in APIConfigs) {
    ...
    APIs[selectedKey] = (params: APIParamsType[typeof selectedKey]) =>
      Fetch(method, endpointPath, params);
  }
...
const Fetch = async (method: string, endpoint: string, params?: any) => {
  ...
  await fetch(`${BaseURL}${endpoint}${additionalURL}`, {
    method,
    headers: {
